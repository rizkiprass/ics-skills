...
Scans AWS account and collects resource information for technical documentation.
"""
...
def main():
    ...
    if len(sys.argv) < 4:
        print("Usage: python scan-aws-resources.py <access_key> <secret_key> <region>")
        sys.exit(1)
    ...
    scanner = AWSResourceScanner(access_key, secret_key, region)
    results = scanner.scan_all()
    
    # Get project root directory
    project_root = get_project_root()
    
    # Save results to JSON file in project root
    output_file = project_root / f"aws-resources-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
